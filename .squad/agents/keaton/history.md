...
### 2026-03-05T[NOW]: Issue #194 — SDK-First Squad Mode Phase 1 Scoping
- **Task:** Analyze 4-phase PRD (#194) and scope what's achievable for v0.8.21 tonight. Brady requested scoping decision.
- **Key finding: Two competing `SquadConfig` types.** `config/schema.ts` has a simpler `SquadConfig` with `defineConfig()` and `validateConfig()`. `runtime/config.ts` has a richer `SquadConfig` (models with fallback chains, casting, platforms, MCP) that `squad.config.ts` at project root actually uses. These must be reconciled — runtime/config.ts is canonical.
- **Key finding: Brady's comment inverts the PRD.** PRD says TS → markdown (generate). Brady says markdown → TS facade (read). Markdown is always the source of truth. Users should never have to run `squad build`. This fundamentally changes the bridge direction.
- **Key finding: No builder functions exist yet.** `defineConfig()` in schema.ts is a shallow merge helper, not a builder in the PRD sense. `defineTeam()`, `defineAgent()`, `defineRouting()`, `defineCeremony()`, `defineHooks()` — all need to be created from scratch.
- **Key finding: OTel runtime is fully built but dead.** 6+ modules in `runtime/` (otel-init, otel-metrics, otel-bridge, event-bus, squad-observer, cost-tracker) are complete but have no activation path. Phase 3 work, not tonight.
- **Scoping verdict:** Phase 1 tonight = builder functions + `squad build --check` (validation only). Defer generation, scaffolding, runtime, migration to later phases. Builder functions are pure identity-with-validation wrappers using runtime/config.ts types. `squad build` validates config consistency, does NOT generate markdown.
- **Decision written to:** `.squad/decisions/inbox/keaton-sdk-mode-phase1-scope.md`
