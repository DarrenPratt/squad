...
### SDK-First builder and build-command tests — Issue #194 Phase 1 (2026-03-05)
**Status:** Complete — 60 new tests across 2 files, all passing.
- **test/builders.test.ts (36 tests):** defineTeam (6), defineAgent (6), defineRouting (5), defineCeremony (4), defineHooks (3), defineCasting (4), defineTelemetry (6), SquadSDKConfig composition (2). Covers valid configs, missing required fields, actionable error messages, default value application, and full config composition.
- **test/build-command.test.ts (24 tests):** File generation (4), generated header verification (4), protected file safety (4), --check mode (4), --dry-run mode (4), edge cases (4). Uses real filesystem with mkdtempSync/rmSync cleanup.
- **Pattern:** Contract-first (spec-driven) tests with local stubs. Types imported from real `packages/squad-sdk/src/builders/types.ts`. Builder function stubs and build pipeline stubs are inline — clearly marked for replacement when Edie's and Fenster's implementations land.
- **Key decisions:** Tests validate the PRD contract (`<!-- generated by squad build -->` header, protected files list, --check exit codes, --dry-run no-write guarantee). Stubs implement minimal validation logic so tests are green today and serve as acceptance criteria for implementation.
- **Protected files verified:** decisions.md, history.md, orchestration-log/ — all confirmed untouched by build.